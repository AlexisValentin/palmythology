...
function analyzeCards(cards: StoryblokCard[]): {
	...
	for (const card of cards) {
		...
		const report: CardReport = {
			...
			hasQuotations:
				Array.isArray(content.quotations) && content.quotations.length > 0,
			quotationsCount: Array.isArray(content.quotations)
				? content.quotations.length
				: 0,
		};
...
function printReport(results: {
	...
	console.log("-".repeat(80));
	console.log(
		`1. CARDS WITH NO SUMMARY (mdSummary = "???" or empty): ${noSummary.length} cards`,
	);
	console.log("-".repeat(80));
	console.log();
	...
	if (noSummary.length === 0) {
		...
		for (const [pantheon, cards] of Object.entries(byPantheon)) {
			...
			for (const card of cards) {
				...
				const quotStatus = card.hasQuotations
					? `Quotes: ${card.quotationsCount}`
					: "Quotes: NONE";
	...
	console.log("-".repeat(80));
	console.log(
		`2. CARDS WITH SUMMARY BUT MISSING FAQ/QUOTATIONS: ${hasSummaryButMissingContent.length} cards`,
	);
	console.log("-".repeat(80));
	console.log();
	...
	if (hasSummaryButMissingContent.length === 0) {
		...
		const missingBoth = hasSummaryButMissingContent.filter(
			(c) => !c.hasFaq && !c.hasQuotations,
		);
		const missingFaqOnly = hasSummaryButMissingContent.filter(
			(c) => !c.hasFaq && c.hasQuotations,
		);
		const missingQuotationsOnly = hasSummaryButMissingContent.filter(
			(c) => c.hasFaq && !c.hasQuotations,
		);

		if (missingBoth.length > 0) {
			console.log(
				`   [MISSING BOTH FAQ AND QUOTATIONS] (${missingBoth.length} cards)`,
			);
		...
		if (missingFaqOnly.length > 0) {
			...
			for (const [pantheon, cards] of Object.entries(byPantheon)) {
				...
				for (const card of cards) {
					console.log(
						`         - ${card.name} (has ${card.quotationsCount} quotes)`,
					);
				}
			}
			console.log();
		...
		if (missingQuotationsOnly.length > 0) {
			console.log(
				`   [MISSING QUOTATIONS ONLY] (${missingQuotationsOnly.length} cards)`,
			);
	...
	console.log(
		`   Cards needing FAQ/Quotations:      ${hasSummaryButMissingContent.length}`,
	);
	console.log(
		`   TOTAL cards needing attention:     ${noSummary.length + hasSummaryButMissingContent.length}`,
	);
	console.log("=".repeat(80));
}
